...
bool Win32GetDnsSuffix(char *domain, UINT size)
{
	...
	if (GetAdaptersAddresses(AF_INET, 0, NULL, (IP_ADAPTER_ADDRESSES *)info, &info_size) == ERROR_BUFFER_OVERFLOW)
	{
		Free(info);
		info = ZeroMalloc(info_size);
	}
	if (GetAdaptersAddresses(AF_INET, 0, NULL, (IP_ADAPTER_ADDRESSES *)info, &info_size) != NO_ERROR)
	{
		Free(info);
		return false;
	}
